...
const Vans = () => {
  ...
  return (
    <div className="bg-orange-50  sm:p-10 p-5">
        ...
        {Vans &&
          Vans.map((item, index) => (
            ...
            <div key={item.id} className="max-w-sm mb-3">
              ...
              <button className={item.type === "simple" ? " py-1 w-16 text-white capitalize rounded text-sm bg-orange-500" : item.type === "rugged" ? "py-1 w-16 text-white capitalize rounded text-sm bg-green-800" : item.type === "luxury" ? "py-1 w-16 text-white capitalize rounded text-sm bg-black" : ""}>
                {item.type}
              </button>
            </div>
          ))}
